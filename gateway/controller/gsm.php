...
class gsm extends ujn_controller {
	...
	function GpsToAddress(){
		...
		//入库
		$result = ( !empty($AddressZH) && mdl_log_gsm::getInstance()->ParseGpsToAddressLog($latitude,$longitude,$imei,$AddressZH)) ?
					array('state'=>"1",'message'=>$AddressZH) :
					array('state'=>"-1001",'message'=>"解析失败");
		output_json($result);
	}
